...
data = read.csv("clinics2.csv")   ####
data = data[196:nrow(data),]
data$latitude = as.character(data$latitude)
data$longitude = as.character(data$longitude)
data$cor = paste(data$latitude,data$longitude,sep = ",")
dest = data$cor[1]
...
colnames(data) = c("DVS/Clinics",index_names_dest) ###
data$`DVS/Clinics` = index_names_origin    ###
#write.csv(data,file = ".\\DVS-Clinics.csv",row.names = F)   ###
final = read.csv("DVS-Clinics.csv")
final = cbind(final,data[,-1])
write.csv(final,file = ".\\DVS-Clinics.csv",row.names = F) 
